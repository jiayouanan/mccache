...
from MMCache.cache_alg import SingleAlg
from MMCache.reorder_alg import ReorderAlg
import numpy as np
import json
import pdb
import os
...
class WorkloadTest(WorkloadManager):
    ...
    def batch_test(self, queue: Queue, csize: float, cache_scheme: str, alg_name: str, batch_size: int, reorder_flag:bool, liveness_bound:int, reorder_name:str, opt_len:int, ob_acc:float) -> dict:
        ...
        alg_obj.workload_init(cache_size, cache_scheme, item_num=self.workload_stats['query_num'], findOB_trunc=opt_len, ob_acc=ob_acc)
        ...
        for i in range(batch_num):
            ...
            # to record reorder alg total time
            ...
            reorder_time = reorder_time + (time.time()-reorder_start_time)
            # print(reorder_time)
           ...
           # print("the consumed time after reorder", time.time()-alg_start_time)
        queue.put(None)
        ...
        print('reorder Total Time: {}'.format(reorder_time))
