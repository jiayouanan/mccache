 ...
 */
public class BasicTxnDB extends BasicDB {
  ...
  /**
   * Multiple read queries as a transaction. Result stored in each TCacheQuery.
   */
  @Override
  public Status readtxn(ArrayList<TCacheQuery> readlist) {
    // note: verbose is true by default for BasicTxnDB
    if (verbose) {
      StringBuilder sb = getStringBuilder();
      sb.append("Transaction Start \n");
      // append each read query to transaction.
      for (TCacheQuery readqry : readlist) {
        delay();
        String table = readqry.getTable();
        String key = readqry.getKeyName();
        Set<String> fields = readqry.getFields();
        ...
        sb.append("]\n"); // adding '\n' to the end of each query in transaction
        // increase counter if count is true
        if (count) {
  ...
  /**
   * Multiple write queries as a transaction. Result stored in each TCacheQuery.
   */
  @Override
  public Status writetxn(ArrayList<TCacheQuery> writelist) {
    if (verbose) {
      ...
      sb.append("Transaction Start \n");
      // append each read query to transaction.
      for (TCacheQuery writeqry : writelist) {
        delay();
        String table = writeqry.getTable();
        String key = writeqry.getKeyName();
        Map<String, ByteIterator> values = writeqry.getValues();
