...
public class BasicDB extends DB {
  ...
  public Status readtxn(ArrayList<TCacheQuery> readlist) {
    ...
    for (TCacheQuery readquery : readlist) {
      read(readquery.getTable(), readquery.getKeyName(), 
          readquery.getFields(), readquery.getValues());
    }
    if (verbose) {
  ...
  public Status writetxn(ArrayList<TCacheQuery> writelist) {
    ...
    for (TCacheQuery writequery : writelist) {
      update(writequery.getTable(), writequery.getKeyName(), writequery.getValues());
    }
    if (verbose) {
      System.out.println("Transaction End");
    }
