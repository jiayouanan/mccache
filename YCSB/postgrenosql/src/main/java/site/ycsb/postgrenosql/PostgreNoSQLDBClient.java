...
public class PostgreNoSQLDBClient extends DB {
  ...
  public void init() throws DBException {
    ...
    synchronized (PostgreNoSQLDBClient.class) {
      ...
      // Added by W.Z
      printExe = props.getProperty(PRINT_OR_EXE, DEFAULT_PROP);
      // End of modification by W.Z
      ...
      try {
        ...
        // for debug
        System.out.println("connection urls: " + urls);
  ...
  public Status readtxn(ArrayList<TCacheQuery> readlist) {
    // @TODO: try to collect transaction result?
    try {
      // disable auto commit for read transaction
      if (connection.getAutoCommit()) {
      ...
      // prepare read query statement one by one
      for (TCacheQuery readquery : readlist) {
        String tableName = readquery.getTable();
        String key = readquery.getKeyName();
        Set<String> fields = readquery.getFields();
      ...
      LOG.error("Error in processing readtxn: " + e);
      return Status.ERROR;
    }
  }
  ...
  public Status writetxn(ArrayList<TCacheQuery> writelist) {
    try {
      ...
      // prepare write query statement one by one
      for (TCacheQuery writequery : writelist) {
        String tableName = writequery.getTable();
        String key = writequery.getKeyName();
        Map<String, ByteIterator> values = writequery.getValues();
      ...
      LOG.error("Error in processing writetxn: " + e);
      return Status.ERROR;
    }
  }
  // modification ended
  ...
  private PreparedStatement createAndCacheInsertStatement(StatementType insertType)
    ...
    if (statement == null) {
      System.out.println("insertStatement: " + insertStatement);  // debug only
