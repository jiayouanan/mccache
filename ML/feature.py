...
def generate_read_features(time_step, batch_start, batch_end, txn_id_seq, txn_item_dict, 
    ...
    slice_read_time_df['read_arrive_times_p200'] = slice_read_time_df.iloc[:,0].apply(lambda r: np.sum(list(map(lambda i: i > time_step-100, [x for x in r if x < time_step]))))        
...
#    slice_write_time_df['write_arrive_times_p10'] = slice_write_time_df.iloc[:,0].apply(lambda r: np.sum(list(map(lambda i: i > time_step-10, [x for x in r if x < time_step]))))
    ...
    slice_write_time_df['write_arrive_times_p200'] = slice_write_time_df.iloc[:,0].apply(lambda r: np.sum(list(map(lambda i: i > time_step-100, [x for x in r if x < time_step]))))        
